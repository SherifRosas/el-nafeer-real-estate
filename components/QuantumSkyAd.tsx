...
// --- IMPERIAL SKY STANDARD V110.15 (FULL-SCREEN MERGE) ---

const GOLD = "#c5a059";

export default function QuantumSkyAd() {
    ...
    useEffect(() => {
        ...
        trackInteraction('OPEN_SKY_V1', window.location.href);
                    ...
                    >
                        ...
                        <div className="absolute left-1/2 -translate-x-1/2 top-[4vh] z-[999999] w-[95%] max-w-[450px] px-4 flex justify-around items-center gap-4">
                            <motion.a href="tel:+201070615372" title="Call Us" initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}>
                                <div onClick={() => handleAction('CALLING', 'tel:+201070615372')} className="w-14 h-14 rounded-full bg-black/40 backdrop-blur-xl border border-cyan-500/30 flex items-center justify-center shadow-2xl hover:bg-cyan-500/20 transition-all group active:scale-90">
                                    <Phone className="w-7 h-7 text-cyan-400" />
                                </div>
                            </motion.a>

                            <motion.a href="https://wa.me/201070615372" title="WhatsApp" initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}>
                                <div onClick={() => handleAction('WHATSAPP', 'https://wa.me/201070615372')} className="w-14 h-14 rounded-full bg-black/40 backdrop-blur-xl border border-green-500/30 flex items-center justify-center shadow-2xl hover:bg-green-500/20 transition-all group active:scale-90">
                                    <MessageCircle className="w-7 h-7 text-green-400" />
                                </div>
                            </motion.a>

                            <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="cursor-pointer">
                                <div onClick={() => setShowQR(true)} className="w-14 h-14 rounded-full bg-black/40 backdrop-blur-xl border border-white/20 flex items-center justify-center shadow-2xl hover:bg-white/10 transition-all group active:scale-90">
                                    <QrCode className="w-7 h-7 text-white/70 group-hover:text-white" />
                                </div>
                            </motion.div>

                            <motion.a href="https://maps.app.goo.gl/r6vGf" target="_blank" title="Location" initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.5 }}>
                                <div onClick={() => handleAction('LOCATION', 'https://maps.app.goo.gl/r6vGf')} className="w-14 h-14 rounded-full bg-black/40 backdrop-blur-xl border border-[#c5a059]/30 flex items-center justify-center shadow-2xl hover:bg-[#c5a059]/20 transition-all group active:scale-90">
                ...
                {showQR && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[1000000] bg-black/90 backdrop-blur-2xl flex items-center justify-center p-6">
                        <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="relative bg-zinc-900 border border-white/10 p-8 rounded-[2rem] max-w-sm w-full text-center shadow-[0_0_100px_rgba(255,255,255,0.05)]">
                            ...
                            <h3 className="text-xl font-bold text-white mb-2 uppercase tracking-widest font-serif">Imperial Access</h3>
                            <p className="text-zinc-500 text-xs mb-8 tracking-widest uppercase">Scan to enter simulation</p>
                            ...
                            <p className="mt-8 text-[10px] text-[#c5a059] tracking-[0.3em] font-black uppercase italic">Lever Pioneer | Ultima</p>
                        </motion.div>
                    </motion.div>
                )}
            </AnimatePresence>

            {/* --- MUTE RING (AUDIO TOGGLE V110.15) --- */}
            {phase === 'active' && (
            ...
            {phase === 'active' && (
                <div className="fixed right-[4%] bottom-[4%] z-[999999]">
                    <a href="tel:+201065661882" className="block active:scale-95 transition-transform">
                        <div onClick={() => handleAction('SIGNATURE_DIRECT', 'tel:+201065661882')} className="cursor-pointer bg-black/90 rounded-2xl p-4 border border-white/10 flex items-center gap-4">
                             ...
                             <div className="w-10 h-10 rounded-full border-2 border-[#c5a059]/50 flex items-center justify-center bg-black/40"><Phone className="w-4 h-4 text-[#c5a059]" /></div>
                        </div>
                    </a>
                </div>
            )}
