...
// --- IMPERIAL SUPREMACIST V39.0 (SINGLE SURFACE SOVEREIGNTY) ---
...
export default function AdClient() {
    ...
    return (
        <div className="fixed inset-0 z-[1000] bg-black flex items-center justify-center p-0 m-0 overflow-hidden select-none font-sans">
                ...
                <div onClick={startUltimaSequence} className="fixed inset-0 z-[5000] bg-black flex flex-col items-center justify-center cursor-pointer p-6">
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center gap-24">
                        ...
                        <div className="flex flex-col items-center gap-6 text-center text-white">
                             ...
                             <p className="text-sahara-gold font-bold text-xl tracking-[0.3em] animate-pulse uppercase">[ ACCESS CELEBRATION ]</p>
                        </div>
                    </motion.div>
                </div>
            )}
            ...
            <div className={`relative w-full h-full flex flex-col items-center justify-center transition-all duration-1500 ${phase === 'active' || phase === 'stabilizing' ? 'opacity-100' : 'opacity-0'}`}>
                ...
                <div className="relative w-full h-full flex items-center justify-center overflow-hidden z-10 pointer-events-none">
                     ...
                     <motion.div animate={{ scale: [1, 1.04 + (audioIntensity * 0.02), 1] }} className="relative w-full h-full flex items-center justify-center">
                         <div className="relative w-full h-auto max-h-[96vh] aspect-square flex items-center justify-center px-4 overflow-hidden">
                            ...
                            <div className="absolute inset-0 z-20 pointer-events-none flex items-center justify-center border border-white/0">
                ...
                {/* --- SOVEREIGN INTERACTION LAYER (Z-100) --- */}
                {phase === 'active' && (
                    <div className="fixed inset-0 z-[100] h-full w-full pointer-events-auto flex flex-col justify-between cursor-default">
                         ...
                         <div className="p-8 flex justify-between items-start text-cyan-400/40 text-[10px] robotic-digits tracking-[5px] uppercase font-black pointer-events-none">
                              <div className="flex gap-4 items-center">
                                   ...
                                   <span>v39.0_SUPREME</span>
                              </div>
                              <Radio className="w-6 h-6 animate-pulse text-red-500" />
                         </div>

                         {/* THE INFALLIBLE TOUCH GRID */}
                         <div className="absolute left-0 bottom-0 w-full h-[40%] flex">
                              <div className="w-1/2 h-full flex flex-col p-4 pl-[5%] pb-[8%]">
                                   <div onClick={() => handleAction('https://wa.me/201111171368')} className="h-1/3 w-full cursor-pointer hover:bg-cyan-400/5 transition-colors duration-300" />
                                   <div onClick={() => handleAction('tel:+201111171368', true)} className="h-1/3 w-full cursor-pointer hover:bg-cyan-400/5 transition-colors duration-300" />
                                   <div onClick={() => handleAction('https://www.google.com/maps?q=29.9656242,31.0922895')} className="h-1/3 w-full cursor-pointer hover:bg-cyan-400/5 transition-colors duration-300" />
                              </div>
                              <div className="w-1/2 h-full flex items-end justify-end p-8 pb-16">
                                   <div onClick={() => handleAction('tel:+201065661882', true)} className="flex items-center gap-4 cursor-pointer group">
