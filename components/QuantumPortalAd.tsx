...
export default function QuantumPortalAd() {
    ...
    return (
        <div className="fixed inset-0 bg-black text-white font-sans overflow-hidden select-none touch-none" style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%' }}>
            ...
            {/* --- THE CINEMATIC GATE (HYDRATION-FRIENDLY) --- */}
            <AnimatePresence>
                {!isStarted && (
                    <motion.div 
                        key="gate"
                        ...
                        className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-10"
                    >
                        <div className="max-w-xl text-center">
                            <motion.div 
                                initial={{ y: 20, opacity: 0 }} 
                                animate={{ y: 0, opacity: 1 }} 
                                className="inline-flex items-center gap-4 px-6 py-2 rounded-full border border-white/10 bg-black/40 mb-12"
                            >
                            ...
                            >
                                <div className="absolute inset-0 bg-cyan-500 translate-x-[-101%] group-hover:translate-x-0 transition-transform duration-500" />
                                <span className="relative z-10">BEGIN_ASCENT</span>
                                <ChevronRight className="relative z-10 inline-block ml-4 w-4 h-4" />
                            </motion.button>
                        </div>
                    </motion.div>
                )}
