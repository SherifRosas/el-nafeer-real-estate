    ...
    try {
      ...
      if (data.success && data.response) {
        ...
        const errorMsg = isArabic
          ? 'عذراً، حدث خطأ. يرجى المحاولة مرة أخرى لاحقاً أو التواصل معنا هاتفياً.'
          : 'I apologize, but I encountered an error. Please try again later or contact us directly.'
        setMessages((prev) => [
          ...prev,
      ...
      if (error.name === 'AbortError' || error.message?.includes('timeout')) {
        const timeoutMsg = isArabic
          ? 'استغرق الرد وقتاً طويلاً. يرجى المحاولة مرة أخرى أو تصفح العقارات المتاحة على الصفحة الرئيسية.'
          : 'The response took too long. Please try again or browse available properties on the home page.'
        setMessages((prev) => [
          ...prev,
    ...
    <>
      {isOpen && (
        <div className="fixed bottom-24 right-4 w-96 max-w-[calc(100vw-2rem)] h-[550px] bg-white rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.3)] flex flex-col z-50 border border-gray-100 overflow-hidden">
          <div className="bg-gradient-to-r from-gray-900 to-black text-white p-5 flex justify-between items-center border-b border-white/10">
              ...
              <div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[10px] text-cyan-500/60 uppercase font-black tracking-widest">
                    {isArabic ? 'متصل الآن' : 'Core Sync Active'}
                  </span>
                </div>
              </div>
            </div>
