...
// --- IMPERIAL KINETIC CINEMA V19.0 (SMART CINEMATIC FIT) ---
...
export default function AdV2UltimaKineticCinema() {
    ...
    // OMNI-AUDIO TRIGGER: Forces sound on first interaction
    const forceAudio = () => {
        ...
        if (playPromise !== undefined) {
            playPromise.catch(() => {
                // Keep trying on any future interaction if it fails here
                console.log("Audio waiting for stronger interaction...");
            });
        }
    }
    ...
    return (
        <div 
            onClick={forceAudio} // Omni-Trigger
            onTouchStart={forceAudio} // Omni-Trigger
        ...
        >
            ...
            {!isAudioUnlocked && (
                <div onClick={startUltimaSequence} className="fixed inset-0 z-[1000] bg-black flex flex-col items-center justify-center cursor-pointer overflow-hidden px-6">
                    ...
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative z-10 flex flex-col items-center gap-20">
                        <div className="relative w-64 h-64 flex items-center justify-center">
                             <motion.div animate={{ rotate: 360 }} transition={{ duration: 15, repeat: Infinity, ease: "linear" }} className="absolute inset-[-40px] border border-cyan-400/20 [clip-path:polygon(25%_0%,75%_0%,100%_50%,75%_100%,25%_100%,0%_50%)]" />
                             <motion.div animate={{ y: [-150, 150] }} transition={{ duration: 1.5, repeat: Infinity, ease: "linear" }} className="absolute left-[-40px] right-[-40px] h-[2px] bg-cyan-400 shadow-[0_0_20px_rgba(34,211,238,1)] z-40" />
                             <div className="w-full h-full relative rounded-full border-2 border-cyan-400/40 bg-black/40 backdrop-blur-3xl flex items-center justify-center p-12 overflow-hidden">
                                <NextImage 
                                    src="/clients/lever-pioneer/logo_mimic.png" 
                                    alt="Logo" width={400} height={400} 
                                    className="object-contain invert brightness-150 contrast-200 mix-blend-screen opacity-90"
                                />
                             </div>
                        </div>

                        <div className="text-center space-y-12">
                             <h1 className="text-white font-black text-4xl lg:text-6xl tracking-[0.4em] robotic-digits uppercase skew-x-[-10deg]">SENTIENT_LINK</h1>
                             <div className="flex flex-col items-center gap-6">
                                <p className="text-sahara-gold font-black text-2xl tracking-[0.3em] uppercase animate-pulse">[ TAP SCREEN TO BROADCAST ]</p>
                                <div className="flex items-center gap-4 text-cyan-400/40 text-[10px] robotic-digits">
                                    <Activity className="w-4 h-4 animate-bounce" />
                                    <span>SYSTEM_CALIBRATING_OMNI_AUDIO_v19.0</span>
                                </div>
                             </div>
                        </div>
                    </motion.div>
            ...
            <AnimatePresence>
                {phase === 'descent' && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-[200] bg-black flex flex-col items-center justify-center text-center">
                        <h2 className="text-white font-black text-4xl tracking-[2rem] robotic-digits uppercase animate-pulse">SMART_FIT_19.0</h2>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <div className={`relative w-full h-full flex flex-col items-center justify-center transition-all duration-1500 ${phase === 'active' || phase === 'stabilizing' ? 'opacity-100' : 'opacity-0'}`}>
                ...
                {/* 
                    SMART CINEMATIC ADAPTER:
                    1) Blurred Backdrop fills the phone height
                    2) Original Ad is Contained in the foreground - 100% VISIBLE
                */}
                <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
                     ...
                     <div className="absolute inset-0 z-0">
                         <NextImage 
                            ...
                            alt="Blur" fill className="object-cover scale-150 blur-3xl opacity-40 brightness-[0.3]" 
                         />
                     </div>

                     {/* 2. FOREGROUND AD (100% VISIBLE) */}
                     <motion.div 
                     ...
                     >
                         <div className="relative w-full h-auto aspect-square flex items-center justify-center">
                            <NextImage 
                                src="/campaigns/lever-pioneer/ad-v2-quantum.png" 
                                alt="Main Ad" 
                                fill className="object-contain drop-shadow-[0_0_50px_rgba(34,211,238,0.3)]" 
                                priority 
                            />

                            {/* HUD LAYER */}
                            <div className="absolute left-[5%] bottom-[5%] lg:left-[8%] lg:bottom-[10%] z-40 pointer-events-none flex flex-col items-start scale-[0.55] lg:scale-100 origin-bottom-left">
                                <motion.div initial={{ x: -100, opacity: 0 }} animate={step >= 2 ? { x: 0, opacity: 1 } : {}} transition={{ duration: 2 }} className="w-[420px] lg:w-[480px] flex flex-col gap-36 pointer-events-auto">
                                    <div className="flex items-center gap-8 cursor-pointer" onClick={() => window.open('https://wa.me/201111171368', '_blank')}>
                                        <div className="w-24 h-24 rounded-full border-2 border-green-500 flex items-center justify-center shadow-[0_0_50px_rgba(34,197,94,0.5)] bg-black/80">
                                    ...
                                    <div className="flex items-center gap-8 cursor-pointer" onClick={() => window.open('tel:19XXX', '_self')}>
                                        <div className="w-24 h-24 rounded-full border-2 border-cyan-400 flex items-center justify-center shadow-[0_0_50px_rgba(34,211,238,0.5)] bg-black/80">
                                    ...
                                    <div className="flex items-center gap-8 cursor-pointer" onClick={() => window.open('https://www.google.com/maps?q=29.9656242,31.0922895', '_blank')}>
                                        <div className="w-24 h-24 rounded-full border-2 border-sahara-gold flex items-center justify-center shadow-[0_0_50px_rgba(212,175,55,0.5)] bg-black/80">
                                            <MapPin className="w-12 h-12 text-sahara-gold" />
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-[14px] text-sahara-gold font-black uppercase">GIZA_HQ</span>
                ...
                <div className="absolute inset-0 pointer-events-none z-50 p-6 lg:p-12 flex flex-col justify-between">
                     <div className="flex justify-between items-start text-cyan-400/60 text-[10px] robotic-digits tracking-[5px]">
                         <div className="flex gap-4 items-center">
                              ...
                              <span>SYSTEM_v19.0_SMART_FIT</span>
                         </div>
